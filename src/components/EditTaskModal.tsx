...
export function EditTaskModal({ open, onOpenChange, onSubmit, task, categories }) {
  const [name, setName] = useState("");
  const [category, setCategory] = useState("");
  const [priority, setPriority] = useState("Medium");
  const [due, setDue] = useState("");

  // When task or open state changes, update the form fields
  useEffect(() => {
    if (task && open) {
      setName(task.name || "");
  ...
  function handleSubmit(e) {
    ...
    if (!name.trim()) return;
    onSubmit({
      ...task, // Keep the original task properties (like id)
      name,
      category,
      priority,
      due
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-xl px-3 py-8 gap-3">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <h2 className="font-bold text-lg text-purple-800">Edit Task</h2>
          <div>
